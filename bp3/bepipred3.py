...
class Antigens():
    def __init__(self, fasta_file, esm_encoding_dir,
        add_seq_len=False, run_esm_model_local=None, compute_contacts=False):
        """
    ...
    def check_accepted_AAs(self, accs, sequences):
        accepted_AAs = set(['L', 'A', 'G', 'V', 'S', 'E', 'R', 'T', 'I', 'D', 'P', 'K', 'Q', 'N', 'F', 'Y', 'M', 'H', 'W', 'C', 'X', 'B', 'U', 'Z', 'O', '.', '-'])
    ...
    def get_esm2_represention_on_accs_seqs(self):
        ...
        if self.compute_contacts:
            contact_savedir = self.esm_encoding_dir / "contacts"
            if not contact_savedir.is_dir(): contact_savedir.mkdir(parents=True)
        ...
        for b in batch_generator:
            ...
            with torch.no_grad():
                results = model(batch_tokens, repr_layers=[33], return_contacts=self.compute_contacts)
            
            token_representations = results["representations"][33]

            if self.compute_contacts: contact_representations = results["contacts"]
        
            # save encoding sequences
            for i, tokens_len in enumerate(batch_lens):
                ...
                if self.compute_contacts:
                    contact_path = contact_savedir / f"{acc_names[i]}_{enc_id}.pt"
                    contact_representation = contact_representations[i, 0 : tokens_len]
                    torch.save(contact_representation, contact_path)
